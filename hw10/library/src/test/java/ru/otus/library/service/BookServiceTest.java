...
public class BookServiceTest {
    ...
    void shouldReturnAllCommentsByBookId() {
        Author author = new Author(1, BOOK_AUTHOR);
        Genre genre = new Genre(1, BOOK_GENRE);
        given(bookRepository.findById(anyLong())).willReturn(
                Optional.of(
                    new Book(
                            BOOK_ID,
                            BOOK_TITLE,
                            ...
                            genre,
                            List.of(
                                    new Comment(1, "Interesting Book", new Book(BOOK_ID, BOOK_TITLE, author, genre, null)),
                                    new Comment(2, "Boring Book", new Book(BOOK_ID, BOOK_TITLE, author, genre, null))
                            )
                    )
                )
        );
        BookCommentDto comments = bookService.getAllCommentsByBookId(BOOK_ID);
        assertThat(comments)
                .isNotNull();
    }
    ...
    public void createBookWithNonExistingGenre() {
        ...
        assertThrows(EntityNotFoundException.class,
                () -> bookService.saveBook(new BookDto(null, BOOK_TITLE, new AuthorDto(1L, "Jules Verne"), new GenreDto(anyLong(), "tech")))
        );
    }
    ...
    public void updateBookWithNonExistingAuthor() {
        ...
        assertThrows(EntityNotFoundException.class,
                () -> bookService.saveBook(new BookDto(null, BOOK_TITLE, new AuthorDto(anyLong(), "Jules Verne"), new GenreDto(1L, "tech")))
        );
    }
}
